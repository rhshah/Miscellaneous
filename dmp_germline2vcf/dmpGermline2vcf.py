...
logging.basicConfig(
        ...
        level=logging.DEBUG)
logger = logging.getLogger('dmp2vcf')
try:
    import coloredlogs
    coloredlogs.install(level='DEBUG')
except ImportError:
    logger.warning("dmp2vcf: coloredlogs is not installed, please install it if you wish to see color in logs on standard out.")
    pass

try:
    import pandas as pd
except ImportError, e:
    logger.warning("dmp2vcf: pandas is not installed, please install pandas as it is required to run the mapping.")
    sys.exit(1)


def main():
    parser = argparse.ArgumentParser(prog='dmp2vcf.py', description='This module converts multi-sample dmp maf like format to tumor/normal vcf', usage='%(prog)s [options]')
...
if __name__ == "__main__":
    ...
    logging.info("dmp2vcf: Elapsed time was %g seconds", totaltime)
